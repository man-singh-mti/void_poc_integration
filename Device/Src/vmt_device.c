...
static int16_t imu_gravity_squ_avg_buff[DEV_IMU_NUM][DEV_IMU_VECTOR_AXIS_N][DEV_IMU_AVG_BUFF_SIZE] =
		{
			0x0 };
...
void adc_sample_finish_cb(adc_seq_t sequence) { //POC3 review for removal
 ...
 uart_tx_channel_undo();
	if (b_water_det_en) {
		if (sequence >= ADC_SEQ_WATER_BEGIN && sequence <= ADC_SEQ_WATER_END) {
			...
			if (b_water_det_log != b_water_det) { //change of state
...
static void dev_icm_pin_set_cb(uint8_t id, icm20948_pin_t pin,
bool b_status) {
	switch (pin) {
...
static void dev_icm_sample_finish_cb(uint8_t id) {
	...
	if (p_h_imu->b_update) {
		...
		//if(id == 2) 
		if(id == 1) { //second IMU read complete
			imu_validate(h_imu);
			if(live_imu) imu_compare(&h_imu[0], &h_imu[1]);
			if(state_get() == measure_state || live_imu){
				if(imu_active_get() == 0) bottom_detect(&h_imu[0]);
				else if(imu_active_get() == 1) bottom_detect(&h_imu[1]);
			}
		}
	}
		...
		if (b_log_save_trigger) {
			...
			for (uint8_t imu_n = 0; imu_n < DEV_IMU_NUM; imu_n++) {
				...
				if(imu_n == 0) {
					...
					if(flash_value.amplitude > flash_value.amplitude_max) flash_value.acc_x_max = flash_value.acc_x;
					flash_value.amplitude = abs(flash_value.acc_y);
					flash_value.amplitude_max = abs(flash_value.acc_y_max);
					if(flash_value.amplitude > flash_value.amplitude_max) flash_value.acc_y_max = flash_value.acc_y;
					flash_value.amplitude = abs(flash_value.acc_z);
					flash_value.amplitude_max = abs(flash_value.acc_z_max);
					if(flash_value.amplitude > flash_value.amplitude_max) flash_value.acc_z_max = flash_value.acc_z;
...
static bool imu_process(h_imu_t *p_h_imu) {
	...
	case STEP_VECTOR_ANGLE: {
		if(p_h_imu->h_accel.len - 2070 < 350 && p_h_imu->h_accel.len - 2070 > -350) { //not accelerating
...
dev_res_t dev_spi_send(h_dev_spi_send_t *p_h_spi) {
	...
	res_hal = HAL_SPI_TransmitReceive_DMA(p_h_spi_hal[p_h_spi->channel],
			p_h_spi->p_tx_buff, p_h_spi->p_rx_buff, p_h_spi->data_size);
