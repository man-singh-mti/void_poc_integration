...
/* function */
__weak void flash_fifo_write_finish_cb(h_flash_fifo_t *p_h_flash,
...
__weak void flash_fifo_mark_set_finish_cb(h_flash_fifo_t *p_h_flash,
...
//			printf("\r\n");
	...
	case STEP_PROGRAM_HEADER_START: {
		flash_res_t res = flash_program_data(p_h_process->pack_addr_header,
				&p_h_process->h_header, sizeof(h_flash_fifo_header_t));
...
//			printf("\r\n");
	...
	case STEP_PROGRAM_DATA_START: {
		flash_res_t res = flash_program_data(p_h_process->pack_addr_data,
				p_h_process->p_data, p_h_process->data_size);
		if (res == FLASH_RES_BUSY)
			break;
...
flash_fifo_res_t flash_fifo_delete_seq(h_flash_fifo_t *p_h_flash, size_t seq) {
	...
	flash_res_t res_program;
	res_program = flash_program_data(addr, (void*) &data_size,
			sizeof(data_size));
