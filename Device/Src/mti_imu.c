...
void imu_value_set (char* var_name, char* var) {
	if(strcmp(var_name, "threshold_air") == 0) threshold_air = atoi(var);
	else if(strcmp(var_name, "threshold_water") == 0) threshold_water = atoi(var);
	else if(strcmp(var_name, "diff") == 0) div_differential = atof(var);
	else if(strcmp(var_name, "max") == 0) div_acc_max = atof(var);
	else if(strcmp(var_name, "ave_sample") == 0) ave_sample = atoi(var);
	else if(strcmp(var_name, "ave_mode") == 0) {
		if(strcmp(var, "y") == 0) ave_mode = 1;
		else if(strcmp(var, "l") == 0) ave_mode = 2;
	}
	else if(strcmp(var_name, "ave_threshold") == 0) threshold_average = atoi(var);
	else {
		printf("@db,Invaid\n");
		return;
	}
...
imu_test_t imu_test(h_imu_t *p_h_imu) {
	...
	if(self_test_complete[id] == 200) return IMU_TEST_COMPLETE;
	for(int i = 9;i > 0;i--) {
		data_FIFO[id][i] = data_FIFO[id][i-1]; //shift data FIFO
	}
	data_FIFO[id][0] = *p_h_imu;
	//data_FIFO[id][0].accel_x = 0;
	if(self_test_complete[id]<20) { //run through a few times before testing
	...
	for(int i = 0;i < 9;i++) { //scan through array for unchanged values
		bool hold = false;
		if(data_FIFO[id][i].accel_x == data_FIFO[id][i+1].accel_x && (data_FIFO[id][i].accel_x == -32768 || data_FIFO[id][i].accel_x == -1 || data_FIFO[id][i].accel_x == 32767)) hold = true;
		if(data_FIFO[id][i].accel_y == data_FIFO[id][i+1].accel_y && (data_FIFO[id][i].accel_y == -32768 || data_FIFO[id][i].accel_y == -1 || data_FIFO[id][i].accel_y == 32767)) hold = true;
		if(data_FIFO[id][i].accel_z == data_FIFO[id][i+1].accel_z && (data_FIFO[id][i].accel_z == -32768 || data_FIFO[id][i].accel_z == -1 || data_FIFO[id][i].accel_z == 32767)) hold = true;
		if(data_FIFO[id][i].gyro_x == data_FIFO[id][i+1].gyro_x && (data_FIFO[id][i].gyro_x == -32768 || data_FIFO[id][i].gyro_x == -1 || data_FIFO[id][i].gyro_x == 32767)) hold = true;
		if(data_FIFO[id][i].gyro_y == data_FIFO[id][i+1].gyro_y && (data_FIFO[id][i].gyro_y == -32768 || data_FIFO[id][i].gyro_y == -1 || data_FIFO[id][i].gyro_y == 32767)) hold = true;
		if(data_FIFO[id][i].gyro_z == data_FIFO[id][i+1].gyro_z && (data_FIFO[id][i].gyro_z == -32768 || data_FIFO[id][i].gyro_z == -1 || data_FIFO[id][i].gyro_y == 32767)) hold = true;
		if(data_FIFO[id][i].mag_x == data_FIFO[id][i+1].mag_x && (data_FIFO[id][i].mag_x == -32768 || data_FIFO[id][i].mag_x == -1 || data_FIFO[id][i].mag_x == 32767)) hold = true;
		if(data_FIFO[id][i].mag_y == data_FIFO[id][i+1].mag_y && (data_FIFO[id][i].mag_y == -32768 || data_FIFO[id][i].mag_y == -1 || data_FIFO[id][i].mag_y == 32767)) hold = true;
		if(data_FIFO[id][i].mag_z == data_FIFO[id][i+1].mag_z && (data_FIFO[id][i].mag_z == -32768 || data_FIFO[id][i].mag_z == -1 || data_FIFO[id][i].mag_z == 32767)) hold = true;
		if(!hold) {
...
void bottom_detect(h_imu_t *p_h_imu) {
	if(samples == 0) startTime = HAL_GetTick();
	
	if (p_h_imu->accel_y == acc_y_last) return;
...
//		samples++;
	...
	if(ave_mode == 1) acc_y_values[0] = p_h_imu->accel_y;
	else if(ave_mode == 2) acc_y_values[0] = p_h_imu->h_accel.len;
	...
	if(p_h_imu->gyro_x > 200 && p_h_imu->gyro_x+365 < -200 && p_h_imu->gyro_z > 200 && p_h_imu->gyro_z < -200) rotating = true;
	else rotating = false;
	...
	if(amplitude < threshold) {
		...
		if(!accelerating_l && !rotating && p_h_imu->h_accel.angle <90) {
			...
			if(angle_delta > 0.5 || angle_delta <-0.5)	angle_x += angle_delta;
			
			angle_delta = (float) p_h_imu->gyro_z*(time_calc)/32768;
			if(angle_delta > 0.5 || angle_delta <-0.5) angle_z += angle_delta;
	...
	uart_tx_channel_set(UART_UPHOLE);
if(HAL_GetTick() - time_pulse > 1500) acc_y_max = 0;
...
//		else messageID++;
	...
	else if(angle_gyro < 20 && p_h_imu->h_accel.angle < 20 && fallen_over) { //return to upright
		fallen_over = false;
//		printf("!tilt,%d,0\r\n",messageID);
//		printf("!tilt,%d,0\r\n",messageID);
//		if(messageID == 9) messageID = 0;
//		else messageID++;
	...
	if(!accelerating_l) { //time stamp change of state of idle
		if(!idle) {
	...
	if(HAL_GetTick() - time_pulse > 480) water_send = false;
	else if(water_get() && water_send && time_pulse + 100 < HAL_GetTick()) { //resend water
		water_send = false;
		keepalive_reset();
		printf("!water,%d,1\r\n",messageID);
		printf("!water,%d,1\r\n",messageID);
...
void bump_detected(void) {
	...
	acc_y_max = amplitude;
	//state = bottom_state;
	if(fallen_over && false) { //fallen over
		if(debug_get()) printf("@db,bump ignored(fallen over)\r\n");
		return;
	} //fallen over
	else{ //not fallen over	
			if(jerk_ratio < 0) jerk_ratio *= -1; //absolute value
		keepalive_reset();
...
void imu_compare(h_imu_t *p_h_imu_1,h_imu_t *p_h_imu_2) {
	...
	switch(live_imu) {
		case 1: printf("@val,%d:%d:%d\r\n",p_h_imu_1->accel_x,p_h_imu_1->accel_y,p_h_imu_1->accel_z); break;
		case 2: printf("@val,%d:%d:%d||%d,%d,%d\r\n",p_h_imu_1->gyro_x,p_h_imu_1->gyro_y,p_h_imu_1->gyro_z,p_h_imu_2->gyro_x,p_h_imu_2->gyro_y,p_h_imu_2->gyro_z); break;
		case 3: printf("@val,%d:%d:%f\r\n",amplitude,acc_y_diff,jerk_ratio); break;
		case 4: printf("@val,%lf:%lf:%f\r\n",p_h_imu_1->h_accel.angle,p_h_imu_2->h_accel.angle,angle_gyro); break;
		case 5: printf("@val,%lf:%lf:%lf:%lf\r\n",inc_x,inc_z,angle_x,angle_z); break;
		case 6: printf("@val,%d:%d:%d||%d,%d,%d\r\n",p_h_imu_1->accel_x,p_h_imu_1->accel_y,p_h_imu_1->accel_z,p_h_imu_2->accel_x,p_h_imu_2->accel_y,p_h_imu_2->accel_z); break;
		case 7: printf("@val,%d\n",p_h_imu_1->accel_y); break;
		case 8: printf("@val,%d\n",p_h_imu_1->accel_y); break;
		case 9: printf("@val,%d,%d,%d\n",diff_acc_x,diff_acc_y,diff_acc_z); break;
		case 10: printf("@val,%d,%d,%d\n",diff_gyro_x,diff_gyro_y,diff_gyro_z);	break;
		case 11: printf("@val,%d:%d:%d\r\n",p_h_imu_1->mag_x,p_h_imu_1->mag_y,p_h_imu_1->mag_z); break;
		case 12: printf("@val,%lf:%f\r\n",p_h_imu_2->h_accel.len,p_h_imu_2->h_gyro.len); break;
		case 13: printf("@val,%.2lf:%.2f:%.2lf:%d:%d\r\n",p_h_imu_2->h_accel.angle,p_h_imu_2->h_accel.len,acc_verticle,p_h_imu_2->accel_y,amplitude); break;
		case 14: printf("@val,%.2lf:%d:%d:%.2lf\r\n",acc_verticle,p_h_imu_2->accel_y,amplitude,p_h_imu_2->h_accel.len); break;
		case 15: printf("@val,%.4f:%.4f:%.4f\r\n",component_x,component_y,component_z); break;
		case 16: printf("@val,%d:%.4f:%d:%.4lf\r\n",p_h_imu_2->accel_y,component_y,amplitude,p_h_imu_2->h_accel.len); break;
		case 17: printf("@val,%d:%d:%d\r\n",p_h_imu_1->gyro_x,p_h_imu_1->gyro_y,p_h_imu_1->gyro_z); break;
		case 18: printf("@val,%d:%d:%d\r\n",p_h_imu_2->gyro_x,p_h_imu_2->gyro_y,p_h_imu_2->gyro_z); break;
		case 19: printf("@val,%.2lf:%.2lf:%.4lf:%.4lf,%.2lf\r\n",angle_x,angle_z,length_x,length_z,angle_gyro); break;
		default: printf("@db,No entry for data set %d\n",live_imu); live_imu = 0; break;
	}
	uart_tx_channel_set(UART_UPHOLE);
}
...
void imu_reset(h_imu_t *p_h_imu_1,h_imu_t *p_h_imu_2) {
	...
	if(debug_get()) printf("@db,Gyro position set: %2.2f:%2.2f\r\n",angle_x,angle_z);
	//b_water = false;
