...
static uint8_t uart_rx_buff[UART_NUMBER][UART_RX_BUFF_LEN] = {
	0 };
static uint8_t uart_tx_buff[UART_NUMBER][UART_TX_BUFF_LEN] = {
	0 };
...
//		uart_init_it(h_uart[uart_n].p_h_hal);
		...
		HAL_UART_Receive_DMA(p_h_uart->p_h_hal, p_h_uart->h_rx.p_buff,
				p_h_uart->h_rx.buff_size);
...
size_t uart_tx_data_len_get(uart_select_t uart_channel) {
	h_uart_tx_t *p_h_uart = &h_uart[uart_channel].h_tx;
	return (p_h_uart->back + p_h_uart->buff_size - p_h_uart->front)
			% p_h_uart->buff_size;
}
...
PUTCHAR_PROTOTYPE {
	...
	if (uart_channel == UART_ALL) {
		for (uart_select_t select = UART_FIRST; select < UART_NUMBER;
				select++) {
