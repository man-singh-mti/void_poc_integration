...
static void moving_avg_u16_update(h_moving_avg_u16_t *p_h_avg,
		uint16_t new_data);
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
	...
	case (size_t) ADC_REG_WATER:
		for (adc_seq_t seq = ADC_SEQ_WATER_BEGIN; seq <= ADC_SEQ_WATER_END;
				seq++) {
...
bool adc_init(adc_hal_seq_t seq) {
	...
	case ADC_HAL_SEQ_TEMP:
		...
		break;
	case ADC_HAL_SEQ_WATER:
		for (adc_seq_t seq_value = ADC_SEQ_WATER_BEGIN;
				seq_value <= ADC_SEQ_WATER_END; seq_value++) {
