...
static char str_head_uphole[STR_H_STR_CMD_HEAD_LEN_GET(h_str_cmd_uphole)] = {
	0x0 };
static char str_head_debug[STR_H_STR_CMD_HEAD_LEN_GET(h_str_cmd_debug)] = {
	0x0 };
static char str_head_void[STR_H_STR_CMD_HEAD_LEN_GET(h_str_cmd_void)] = {
	0x0 };
...
static h_cmd_str_buff_t h_cmd_str_buff[UART_NUMBER] = {
	0x0 };
...
static void cmd_uart_process(uart_select_t ch) {
	...
	size_t str_len = 0;
	uart_rx_dequeue_dma(ch, p_h_buff->front, (uint8_t*) p_buff, CMD_STR_LEN_MAX,
			&str_len);
	p_buff[str_len] = '\0';
...
static void cmd_sensor(h_str_pointers_t *str_p) {	
	uart_tx_channel_set(cmd_uart_ch);	
	if (str_p->part[1] == NULL)	return;	
	//printf("@db,sensor: %c\n",str_p->part[1][0]);
	switch (str_p->part[1][0]) {		
		case 'w': 
			if (str_p->part[2] == NULL) { //calibrate
...
static void cmd_start(h_str_pointers_t *str_p) {
	...
	uart_tx_channel_set(cmd_uart_ch);
	if (dev_log_save_enable_set(auto_log)) printf("@db,Log starting\n");
...
static void cmd_connect(h_str_pointers_t *str_p) { //@connect
	if (str_p->part[1] == NULL) {
		...
		state_set(stopped_state);	
		if(device_init_finish_get()) imu_test_reset();	
		uart_tx_channel_undo();
		return;
	}
...
static void cmd_initial(h_str_pointers_t *str_p) {
	...
	if (str_p->part[1][0] == 'g') {
		...
		cmd_print_init_g_log(cmd_uart_ch);
			 ...
			 if(strcmp(str_p->part[2],"ack") == 0) version_ack(true);
			 else version_ack(false);	
...
//	printf("cmd_log\r\n");
	...
	if (strstr(str_p->part[1], "save") != NULL) {
		if (str_p->part[2] != NULL) {
			...
			if (dev_log_save_enable_set(atoi(str_p->part[2]))) printf("@db,Log enabled\n");
			else printf("@db,Log disabled\n");
		}
...
static void cmd_void(h_str_pointers_t *str_p) {
	...
	uart_tx_channel_set(UART_DEBUG);
	if(void_detected) printf("@db,Void detected\n");
	else printf("@db,Void ended\n");
	uart_tx_channel_set(UART_UPHOLE);
...
//	printf("cmd_debug\r\n");
	...
	else if (strstr(str_p->part[1], "reserve") != NULL) { //set from system config
...
//	printf("\n");
	if(dev_log_report_busy_get()) printf("@db,Exporting flash\n");
	else printf("@da,done\n"); //data export complete
	uart_tx_channel_undo();
}

void cmd_print_log_report_data(uart_select_t channel) { //logEdit
	uart_tx_channel_set(channel);
	h_dev_log_t h_log;
