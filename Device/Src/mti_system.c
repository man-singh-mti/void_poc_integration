...
bool module_init(void) {
	...
	if(HAL_GetTick() < timestamp + 1000) return false;
	...
	switch(init_step) {
		...
		case STEP_VER_SYNC:
			if(version_sent) init_step = STEP_WATER_SYNC;
		...
		case STEP_WATER_SYNC:
			if(reserve_get()) init_step = STEP_IMU_SYNC;
		...
		case STEP_IMU_SYNC:
			if(imu_profile_get() != 0) init_step = STEP_IMU_TEST;
		...
		case STEP_FINISH:
				initialised = true;
				if(module_status == STATUS_SYNC) module_status = STATUS_OK;
	...
	static uint8_t retry;
		...
		if(device_init_finish_get()) {
			...
			if(res[0] == IMU_TEST_ERROR) {
				if(res[1] == IMU_TEST_ERROR) error_code = 2;
				else error_code = 3;
			}
			else if(res[1] == IMU_TEST_ERROR) error_code = 4;
			if(error_code > 1) {
			...
			if(res[0] == IMU_TEST_OK) imu_status[0] = true;
			else if(res[0] == IMU_TEST_ERROR) imu_status[0] = false;
			if(res[1] == IMU_TEST_OK) imu_status[1] = true;
			else if(res[1] == IMU_TEST_ERROR) imu_status[1] = false;
							
				//select which IMU to use based on init status
			if(imu_status[0]) imu_active_set(0);
			else if(imu_status[1]) imu_active_set(1);
			else imu_active_set(30); //invalid
