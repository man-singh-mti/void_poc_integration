...
bool water_detect(void) {
	...
	if(state_get() != measure_state) return b_water;
	
	water_value_1 = adc_value_get(ADC_SEQ_WATER_BEGIN);
	water_value_2 = adc_value_get(ADC_SEQ_WATER_BEGIN+1);
	if(water_value_1 > water_value_2) water_value = water_value_1;
	else water_value = water_value_2;
	if(adc_print) printf("@val,%.0lf,%u,%u\n",adc_value_get(ADC_SEQ_TEMP),water_value_1,water_value_2);
	//printf("@val,%d\n",water_value);
	...
	if(!b_water) {
		if(water_value >water_high_reserve) {
				...
				printf("!water,%d,1\r\n",messageID_water);
				printf("!water,%d,1\r\n",messageID_water);
				if(messageID_water == 19) messageID_water = 10;
				else messageID_water++;
				b_water = true;
				y_max_reset();
			}
		}
		...
		if(water_low > water_high_reserve) water_low = water_high_reserve/water_multiplier;
		if(water_value < water_high_reserve)
			{
...
void reserve_set(uint16_t reserve) {
	if(reserve == 0) water_high_reserve = 1280;
	else water_high_reserve = reserve;
	if(module_status_get() == STATUS_SYNC) module_status_set(STATUS_OK);
}
